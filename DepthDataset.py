...
class NyudDataset(Dataset):
    ...
    def __getitem__(self, item):
        file = self.index[item]
        image = Image.open(os.path.join(file)).convert("L").resize((224, 224))
        real_single_view = np.expand_dims(image, axis=0).astype(np.float32) / 255.0
        return {'real_single_view': real_single_view}